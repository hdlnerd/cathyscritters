...
class JoomBlogModelPosts extends JModelList
{
	...
	protected function getListQuery() 
	{
        ...
        $query->where('c.state BETWEEN 0 AND 1');
		$query->order($db->getEscaped($this->getState('list.ordering', 'c.created')).' '.$db->getEscaped($this->getState('list.direction', 'DESC')));
	...
	{
		$model = JModel::getInstance('Blogs', 'JoomBlogModel', array('ignore_request'=>true));
