	...
	<tbody>
		<tr>
			...
			<td valign="top" width="100%" >
				<form action="<?php echo JRoute::_('index.php?option=com_joomblog&view=posts'); ?>" method="post" name="adminForm" >
					<?php if ($this->posts or $this->categories) { ?>
					<fieldset id="filter-bar">
						<?php if ($this->posts or $search) { ?>
                        ...
                        <div class="filter-select fltrt">
                            <?php 
                                //$tags_array = explode(',', $this->posts['cache']['09a5120b3ebe80823563ef1033bfff92'][0]['tags']);
                                $addcaption = array(JHtml::_('select.option', '', JText::_('COM_JOOMBLOG_FIELD_HEADING_TAG'), 'id', 'name'));
                                $tags_array = ($tags) ? array_merge($addcaption, $tags) : $addcaption;
                                
                                echo JHTML::_('select.genericlist', $tags_array, 'filter_tags_id', 'class="inputbox" onchange="this.form.submit();"', 'id', 'name', $this->state->get('filter.tags_id'));
                            ?>
                        </div>
                        <?php } ?>
					...
					<table class="adminlist">
						...
						<tbody>
						<?php if ($this->posts) {foreach($this->posts as $i => $item) {
							?>
