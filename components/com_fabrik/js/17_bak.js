...
function updatePrice () {
	...
	alert("About to call getSelectedRadio");
  var party_elsewhere = getSelectedRadio($('cc1_critters_reservations___ishomeaddress').getParent('fieldset.fabrikGroup'));
	alert("Just called getSelectedRadio");
  ...
  if (party_elsewhere) {
	...
	newreq = new Request.JSONP(
		{url: url_mq, method: 'post',
			onComplete:
			function(r){
				$('cc1_critters_reservations___distance-one-way').value = Math.round(r.route.distance);
